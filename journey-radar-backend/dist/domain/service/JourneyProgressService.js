...
class JourneyProgressService {
    ...
    async computeProgress(journey, coordinates, journeyId) {
        ...
        const lastRoute = hasRoutes ? journey.routes[journey.routes.length - 1] : firstRoute;
        ...
        const knownStopNameSet = new Set(hardcodedStops_1.HARDCODED_STOPS.map(s => s.name));
        ...
        let selectedRoute = hasRoutes ? journey.routes[selectedRouteIndex] : firstRoute;
    ...
    getNearestStop(latDeg, lonDeg) {
        let best = hardcodedStops_1.HARDCODED_STOPS[0];
    ...
    findStageIndexByNearestStop(stations, nearest) {
    ...
    findNearestStageIndexByDistance(stations, latDeg, lonDeg) {
        // Consider only stations present in the hardcoded list. Choose nearest station and map to connection.
        let bestIdx = -1;
        let bestDist = Number.POSITIVE_INFINITY;
        for (let i = 0; i < stations.length; i++) {
            const s = stations[i];
            const hc = hardcodedStops_1.HARDCODED_STOPS.find(h => h.name === s.name);
            if (!hc)
                continue;
